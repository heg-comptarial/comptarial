...
const [isImpotsDialogOpen, setIsImpotsDialogOpen] = useState(false);
  ...
  useEffect(() => {
        ...
        declaration.rubriques?.flatMap((rubrique) =>
          (rubrique.documents || []).map((doc) => ({
            ...
            annee:
              declaration.annee ||
              new Date(
                declaration.dateCreation || declaration.dateSoumission || ""
              ).getFullYear(),
          }))
        ) || []
    );
    ...
    const filtered = allDocuments.filter(
      (doc) =>
        ...
        (doc.sous_rubrique &&
          doc.sous_rubrique.toLowerCase().includes(searchTerm.toLowerCase())) ||
  ...
  const handleDeclarationStatusChange = async (
    ...
    try {
      ...
      const checkResponse = await fetch(
        `${API_URL}/declarations/${declarationId}/check-documents`
      );
      const checkData = await checkResponse.json();

      if (!checkResponse.ok) {
        toast.error(
          checkData.message || "Erreur lors de la vérification des documents"
        );
        return;
      }
  ...
  const handleDeleteRubrique = async (id: string | number) => {
    ...
    const confirmed = window.confirm(
      "Voulez-vous vraiment supprimer cette rubrique ?"
    );
    if (!confirmed) return;
    ...
    try {
      ...
      const res = await fetch(
        `/api/delete?fileName=${encodeURIComponent(
          fileName
        )}&year=${year}&userId=${userId}&rubriqueName=${encodeURIComponent(
          rubriqueName
        )}`,
        {
          method: "DELETE",
  ...
  const validateDeclarationOnly = async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/declarations/${pendingDeclarationAction.declarationId}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const validateDeclarationAndDocuments = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        toast.error(
          errorData.message ||
            "Erreur lors de la validation de la déclaration et des documents"
        );
        return;
      }
      ...
      <div className="container mx-auto py-10 px-4">
        <div className="flex items-center gap-2 mb-6">
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.push(`/admin/${adminId}`)}
          >
  ...
  <DialogContent>
    ...
    <DialogFooter>
                              ...
                              <h3 className="text-lg font-medium">
                                {activeDeclaration.titre ||
                                  `Déclaration ${
                                    ...
                                    new Date(
                                      activeDeclaration.dateCreation ||
                            ...
                            <Accordion type="multiple" className="w-full">
                              {activeDeclaration.rubriques?.map((rubrique) => (
                                <AccordionItem
                                  ...
                                  value={`rubrique-${
                                    rubrique.rubrique_id || rubrique.id
                                  }`}
                                >
                                ...
                                <TableCell className="text-right">
                                  <div className="flex justify-end gap-1">
                                    ...
                                    <Button
                                      ...
                                      size="icon"
                                      onClick={() =>
                                        handleDeleteDocumentSansRubrique(doc)
                                      }
                                      title="Supprimer le document"
                                    >
                                      <Trash2 className="h-4 w-4" />
      ...
      <AddRubriqueDialog
        ...
        declarationId={
          activeDeclaration?.declaration_id || activeDeclaration?.id
        }
